"""
File: src/training/train_layoutlmv3_vision.py

Train LayoutLMv3 with vision features (Model 3)
"""
...
def load_config():
    ...
    return cfg.get("vision_training", cfg.get("layout_training", {}))


def compute_span_metrics(start_logits, end_logits, start_positions, end_positions):
    """Compute span-level EM and F1"""
    with torch.no_grad():
...
def main():
    ...
    output_dir = Path(cfg.get("output_dir", "models/layoutlmv3_vision/best_model"))
    ...
    print(f"\n{'='*60}")
    print("Model 3: LayoutLMv3 + Vision Features")
    print(f"{'='*60}")
    ...
    # Datasets with vision
    print(f"\nLoading datasets...")
    train_dataset = LayoutLMv3VisionDataset(
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        for step, batch in enumerate(progress_bar):
            ...
            pixel_values = batch["pixel_values"].to(device)  # NEW: Add images
            ...
            outputs = model(
                ...
                bbox=bbox,
                pixel_values=pixel_values,  # NEW: Pass images to model
        ...
        with torch.no_grad():
            for batch in tqdm(val_loader, desc="Validating"):
                ...
                attention_mask = batch["attention_mask"].to(device)
                bbox = batch["bbox"].to(device)
                pixel_values = batch["pixel_values"].to(device)  # NEW
                start_positions = batch["start_positions"].to(device)
                end_positions = batch["end_positions"].to(device)
        ...
        if avg_val_f1 > best_val_f1:
            ...
            print(f"✨ New best span-F1: {best_val_f1 * 100:.2f}% - saving model")
